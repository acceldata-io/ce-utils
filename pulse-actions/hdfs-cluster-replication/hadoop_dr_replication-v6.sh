...
#     "<HDFS_STATE_DIR>"
#
# Default values (used when an argument is omitted or passed as ""):
#   SOURCE_CLUSTER          = production-1.adsre.com:8020   (lab value; always pass explicitly)
#   DEST_CLUSTER            = dr-1.adsre.com:8020           (lab value; always pass explicitly)
#   SOURCE_DIRS             = /demo/oldfiles                (lab value; always pass explicitly)
#   SNAP_PREFIX             = dr_snap
#   SNAP_RETAIN             = 3
#   HDFS_USER               = hdfs
#   DISTCP_USER             = same as HDFS_USER
#   COPY_OPTS               = -strategy dynamic -direct -update -pugptx -skipcrccheck
#   YARN_QUEUE              = default
#   AUTO_FULL_DISTCP        = no
#   ROLLBACK_ON_FAILURE     = no
#   KERBEROS_ENABLED        = no
#   DISTCP_EXCLUDE_PATTERNS = "" (filtering disabled)
#   REPLICATION_MODE        = pull
#   LOG_PATH                = /var/log/hadoop-replication
#   REVERSE_DIFF_BOOTSTRAP  = no
#   HDFS_STATE_DIR          = /tmp/pulse_replication_action
...
#                              Letters, digits, '.', '_' and '-' only; must not start with '-'.
#                              Must be UNIQUE per replication pair for a given source directory: if the
#                              same directory is replicated to two DR clusters, give each job its own
#                              prefix. Two jobs sharing a prefix create, reuse and delete each other's
#                              snapshots. Keep it FIXED for the life of the pair (including across
#                              failover/failback). If it is ever changed, snapshots under the old prefix
#                              are no longer managed by retention and must be deleted by hand on both
#                              clusters (hdfs dfs -deleteSnapshot <dir> <old_prefix>_<N>).
#   5) SNAP_RETAIN          - Number of snapshots to retain on each cluster (integer, e.g. 3)
#                              Retention keeps the SNAP_RETAIN highest-numbered ${SNAP_PREFIX}_<N> snapshots.
#                              A value of 2 or more is recommended: 1 leaves no spare common snapshot.
...
#                                                  Tune -m and -bandwidth together. DistCp's default is
#                                                  100 MB/s per map when -bandwidth is not given.
#                                -m is an UPPER BOUND: a small incremental diff (few changed files) runs
#                                with fewer mappers than requested.
#                                -m, -bandwidth and -strategy values are validated at startup.
...
#                              this string; the script moves them ahead of all DistCp flags automatically.
#                              -update is always applied, whether or not it is listed here.
#                              Rejected (script exits): -delete, -overwrite, -diff, -rdiff, glob characters
#                              (* ? [), and -filters when DISTCP_EXCLUDE_PATTERNS is enabled.
#                              (-delete is still applied, by the script itself, to the full copies --
#                              baseline full DistCp and the baseline reconcile -- so the destination
#                              mirrors the source; see those call sites.)
#                              An empty string selects the default; pass " " (a single space) for no options.
...
#   15) LOG_PATH            - Log directory (default: /var/log/hadoop-replication)
#                              Each replication policy logs to its own subdirectory, one file per run,
#                              with "latest.log" pointing at the current/most recent run:
#                                <LOG_PATH>/<policy>/<YYYYmmdd_HHMMSS>_<pid>.log
#                                <LOG_PATH>/<policy>/latest.log
#                              <policy> = <source>__<dest>__<prefix>__<first dir>[+<N more dirs>]-<hash>
#                              (cluster names without the port; policies that differ only in their
#                              directories get separate subdirectories). Set the LOG_JOB_NAME env var
#                              to use your own policy name instead.
#                              LOG_RETAIN (env var) controls cleanup: default -1 keeps every run log;
#                              a positive N keeps only the newest N per policy. A value ending in ".log"
#                              (or an existing file) selects single-file mode: each policy writes
#                              "<LOG_PATH without .log>.<policy>.log", rotated at the start of each run.
...
#                              Must be an absolute HDFS path without a scheme (no "hdfs://"), with
#                              no whitespace or glob characters (* ? [ ] { } \); the same path is used
#                              on both clusters. A trailing "/" is ignored.
...
#     /var/tmp/dr-last-snap-<SNAP_PREFIX>@<sanitized_dir>.txt are written as a SINGLE line (the last common
...
#     write. State files from earlier versions of this script
#     (/var/tmp/dr-last-snap-<sanitized_dir>.txt, /var/tmp/dr-last-snap-<sanitized_dir>-<SNAP_PREFIX>.txt
#     and the HDFS mirror ${HDFS_STATE_DIR}/dr-last-snap-<sanitized_dir>.txt) are still recognized and
#     migrated to the new name -- but only when the snapshot recorded in them belongs to this job's
#     SNAP_PREFIX, since the old directory-only name was shared by every job replicating the directory.
...
#     ${HDFS_STATE_DIR}/dr-last-snap-<SNAP_PREFIX>@<sanitized_dir>.txt on BOTH the source and
...
#     retry-error files in the policy's log directory; older ones are deleted at startup.
#     Recommended when many policies run on one node.
#     Example: export LOG_RETAIN=30
#   - LOG_JOB_NAME          - Name of this policy's log directory (default: derived, see arg 15).
#     Letters, digits, '.', '_' and '-'; must start with a letter or digit and be unique per policy.
#     Example: export LOG_JOB_NAME=sales_prod_to_dr
...
#                  directory (from the source ${SNAP_PREFIX}_0 snapshot), creates new baseline
#                  snapshot on destination (post-DistCp state). Those directories start
#                  incremental sync on the next run.
#                  WARNING: Can take significant time for large datasets.
#                * If AUTO_FULL_DISTCP="no" (default): instructs the operator to run a
#                  **full DistCp** for each directory (manual step); those directories start
#                  incremental sync on the run after that copy.
#              Either way, every OTHER configured directory continues to Stage 4 in the same run.
...
#   8. Monitor the policy's log: <LOG_PATH>/<policy>/latest.log (15th positional
#      argument: LOG_PATH, default /var/log/hadoop-replication).
#
# Pre-requisites:
...
#
# A log DIRECTORY by default. Each replication policy writes to its own
# subdirectory, one file per run:
#   ${LOG_PATH}/<policy>/<YYYYmmdd_HHMMSS>_<pid>.log
#   ${LOG_PATH}/<policy>/latest.log  -> the current/most recent run
#   <policy> = <source>__<dest>__<prefix>__<first dir>[+<N more dirs>]-<hash>
# (see compute_policy_key; cluster names without the port, and when both clusters
# share one nameservice name, the first NameNode host from SRC_NN_HOSTS/DST_NN_HOSTS
# is appended to each side). Policies between the same two clusters with the same
# SNAP_PREFIX but different SOURCE_DIRS therefore get separate directories, and the
# forward and failover directions of a policy are kept apart.
# LOG_JOB_NAME (env var, optional) replaces <policy> with a name of your choice,
# e.g. the policy name from an orchestrator. It must be unique per policy.
#
# A value ending in ".log", or naming an existing regular file, selects single-file
# mode: each policy writes "<LOG_PATH without .log>.<policy>.log", rotated to
# "<that file>.<YYYYmmdd_HHMMSS>_<pid>" at the start of each run.
# LOG is set by setup_log_file (called first thing in main) to the file this run
# writes to.
#
# The policy directory also holds the snapshotDiff audit file of each rollback
# ("<ts>_rollback_diff_<dir>_from_<snap>_to_<snap>.txt") and the DistCp error output
# of a failed post-rollback retry ("<ts>_distcp_retry_err_<dir>_<pid>.txt").
#
# LOG_RETAIN (env var, default -1): -1 never deletes logs. A positive N keeps the
# newest N run logs per policy (directory mode) or N rotated backups (file mode),
# and the newest N files of each of the two kinds above; older ones are deleted at
# startup. With many policies, setting it is recommended: e.g. 100 policies running
# every 15 minutes create about 9,600 run logs per day.
###############################################################################
LOG_PATH="${15:-/var/log/hadoop-replication}"
LOG=""
LOG_RETAIN="${LOG_RETAIN:--1}"
if ! [[ "$LOG_RETAIN" =~ ^(-1|[1-9][0-9]*)$ ]]; then
    echo "[ERROR] LOG_RETAIN must be -1 (never delete logs) or a positive number of logs to keep: '$LOG_RETAIN'" >&2
    exit 27
fi
LOG_JOB_NAME="${LOG_JOB_NAME:-}"
if [[ -n "$LOG_JOB_NAME" && ! "$LOG_JOB_NAME" =~ ^[A-Za-z0-9][A-Za-z0-9._-]{0,199}$ ]]; then
    echo "[ERROR] LOG_JOB_NAME must start with a letter or digit and contain only letters, digits, '.', '_' and '-' (max 200): '$LOG_JOB_NAME'" >&2
    exit 28
fi
...
# Validate HDFS_STATE_DIR. It must be an ABSOLUTE, scheme-less HDFS path: every access is made as
# "hdfs dfs -fs hdfs://<cluster> ... ${HDFS_STATE_DIR}/<file>" so the same path is used on each cluster.
#   - A relative path resolves against the HDFS home directory of whichever user runs the script (HDFS_USER
#     under sudo, the Kerberos principal otherwise, possibly a different one on another node after failover),
#     so the mirror would not be found in exactly the cross-node case it exists for.
#   - A "hdfs://..." URI overrides "-fs", sending both per-cluster mirror copies to the same cluster.
#   - HDFS CLI commands (-cat, -rm) expand * ? [ ] { } \ as glob patterns, so a -rm could match other files.
#   - Whitespace would split the path in the operator recovery commands this script prints.
# A trailing "/" is dropped so "${HDFS_STATE_DIR}/<file>" never contains "//".
if [[ "$HDFS_STATE_DIR" != /* ]]; then
    echo "[ERROR] HDFS_STATE_DIR (arg 17 / env var) must be an absolute HDFS path starting with '/' (no hdfs:// URI): '$HDFS_STATE_DIR'" >&2
    exit 26
fi
if [[ "$HDFS_STATE_DIR" =~ [*?{}\\[:space:]] || "$HDFS_STATE_DIR" == *[* || "$HDFS_STATE_DIR" == *]* ]]; then
    echo "[ERROR] HDFS_STATE_DIR (arg 17 / env var) must not contain whitespace or the HDFS glob characters * ? [ ] { } \\: '$HDFS_STATE_DIR'" >&2
    exit 26
fi
[[ "$HDFS_STATE_DIR" != "/" ]] && HDFS_STATE_DIR="${HDFS_STATE_DIR%/}"
...
# (resolve_distcp_exclude_file's POLICY_KEY). A space in SNAP_PREFIX would silently split that path into two
...
# Allow-list SNAP_PREFIX to letters, digits, '.', '_' and '-'. The prefix is embedded in regular expressions
# that match snapshot names (snapshot listings, retention cleanup, rollback checks); any other regex
# metacharacter ("+", "(", "[", "{", "^", "$") would stop the script from recognising its own snapshots, and
# an unescaped "." would also match another job's snapshots. "." is allowed and escaped by snap_re().
if [[ ! "$SNAP_PREFIX" =~ ^[A-Za-z0-9._-]+$ ]]; then
    echo "[ERROR] SNAP_PREFIX may only contain letters, digits, '.', '_' and '-': '$SNAP_PREFIX'" >&2
    exit 13
fi

# Escape a snapshot name or prefix for use inside a grep regular expression. After the SNAP_PREFIX allow-list
# above, "." is the only regex metacharacter a snapshot name can contain.
snap_re() {
    printf '%s' "${1//./\\.}"
}
...
# feed compute_policy_key (log directory and DistCp filter file name), which does not strip whitespace either, so
...
# Split COPY_OPTS (arg 8) into Hadoop generic options and DistCp tool options, and reject values DistCp
# would refuse at incremental/rollback time.
#
# Hadoop's GenericOptionsParser stops at the first option it does not recognise, so a generic option
# (-D, -conf, -fs, ...) placed after any DistCp flag (-strategy, -filters, -rdiff, ...) is handed to DistCp,
# which rejects it as "Unrecognized option". Every distcp call site puts COPY_OPTS after DistCp flags, so
# the generic options are pulled out here into COPY_GENERIC_OPTS, which is folded into DISTCP_FULL_OPTS /
# DISTCP_ROLLBACK_FULL_OPTS (placed directly after "hadoop distcp"). COPY_OPTS keeps only DistCp flags.
#
# Rejected flags:
#   -delete / -overwrite : mutually exclusive with -diff/-rdiff and -update respectively; the full copy
#                          would accept them but every incremental sync and every rollback would fail.
...
parse_copy_opts() {
    if [[ "$COPY_OPTS" == *[\*\?\[]* ]]; then
        echo "[ERROR] COPY_OPTS (arg 8) must not contain glob characters ('*', '?', '['): '$COPY_OPTS'" >&2
        exit 23
    fi

    local -a tokens=() generic=() tool=()
    read -r -a tokens <<<"$COPY_OPTS"
    local i t
    for ((i = 0; i < ${#tokens[@]}; i++)); do
        ...
        case "$t" in
            ...
            -D|-conf|-fs|-jt|-files|-libjars|-archives|-tokenCacheFile)
                ...
                ((i++)) || true
                ;;
            -delete|-overwrite|-diff|-rdiff)
                echo "[ERROR] COPY_OPTS (arg 8) must not contain '$t'." >&2
                case "$t" in
                    -delete)    echo "[ERROR] DistCp rejects -delete together with -diff/-rdiff, so every incremental sync and rollback would fail." >&2 ;;
                    -overwrite) echo "[ERROR] DistCp rejects -overwrite together with -update, which incremental sync and rollback require." >&2 ;;
                    *)          echo "[ERROR] Snapshot diff options are managed by this script." >&2 ;;
                esac
                exit 23
                ;;
            -m|-bandwidth|-strategy)
                local v="${tokens[i + 1]:-}"
                case "$t" in
                    -m)
                        if [[ ! "$v" =~ ^[1-9][0-9]*$ ]]; then
                            echo "[ERROR] COPY_OPTS (arg 8): -m needs a positive integer (max map tasks), got '${v:-<missing>}'" >&2
                            exit 23
                        fi
                        ;;
                    -bandwidth)
                        if [[ ! "$v" =~ ^([0-9]+\.?[0-9]*|\.[0-9]+)$ ]] || [[ "$v" =~ ^0*\.?0*$ ]]; then
                            echo "[ERROR] COPY_OPTS (arg 8): -bandwidth needs a positive number (MB/s per map), got '${v:-<missing>}'" >&2
                            exit 23
                        fi
                        ;;
                    -strategy)
                        if [[ "$v" != "dynamic" && "$v" != "uniformsize" ]]; then
                            echo "[ERROR] COPY_OPTS (arg 8): -strategy must be 'dynamic' or 'uniformsize', got '${v:-<missing>}'" >&2
                            exit 23
                        fi
                        ;;
                esac
    ...
    COPY_GENERIC_OPTS="${generic[*]+${generic[*]}}"
    COPY_OPTS="${tool[*]+${tool[*]}}"
...
# State file name for a directory key, shared by the local file (/var/tmp/<name>) and the HDFS mirror
# (${HDFS_STATE_DIR}/<name>). Keyed by SNAP_PREFIX as well as the directory, so two replication jobs for the
# same directory (e.g. one source directory replicated to two DR clusters, each with its own SNAP_PREFIX) never
# share -- or hydrate from -- each other's state. SNAP_PREFIX cannot contain "@" (see its allow-list), so the
# first "@" always separates prefix from key and no two (prefix, directory) pairs map to the same name.
#
# The key is the sanitized directory path, which keeps any character the directory name contains. HDFS CLI
# commands (-cat, -rm) expand * ? [ ] { } \ as glob patterns, so those are encoded -- "~" first, so the
# encoding stays one-to-one and two different directories can never share a name: ~ -> ~t, * -> ~s,
# ? -> ~q, [ -> ~l, ] -> ~r, { -> ~o, } -> ~c, \ -> ~b.
state_file_name() {
    local k="$1"
    # Replacements are held in variables so no bash version applies tilde expansion to "~x".
    local t='~t' st='~s' q='~q' l='~l' r='~r' o='~o' c='~c' b='~b'
    k=${k//\~/$t}
    k=${k//\*/$st}
    k=${k//\?/$q}
    k=${k//\[/$l}
    ...
    k=${k//\}/$c}
    k=${k//\\/$b}
    printf 'dr-last-snap-%s@%s.txt' "$SNAP_PREFIX" "$k"
}

# True if a key contains a character the HDFS CLI treats as a glob. Legacy HDFS mirror names embed the raw
# key, so for such keys the legacy mirrors are neither read nor removed (a glob could match other files).
_key_has_hdfs_glob() {
    [[ "$1" =~ [*?{}\\] || "$1" == *[* || "$1" == *]* ]]
}

# True if a state file's content (line 1: last common snapshot name) belongs to this job's SNAP_PREFIX. Used
# to decide whether a legacy, directory-only-keyed state file is ours to adopt.
_state_content_matches_prefix() {
    local first_line
    first_line=$(printf '%s' "$1" | sed -n '1p')
    [[ "$first_line" =~ ^"$SNAP_PREFIX"_[0-9]+$ ]]
}

# Read the same-named state mirror from both clusters and pick one. Sets PICKED_MIRROR_CONTENT (empty if
# neither cluster has a usable copy). With $2="match-prefix", a copy is usable only if its snapshot belongs to
# this job's SNAP_PREFIX (legacy names are shared by every job for the directory).
#
# The two clusters' copies can legitimately disagree if a prior write's mirror-to-one-cluster step failed
# (best-effort, see mirror_state_file_to_hdfs) or the process died between the two writes. State only ever
# advances forward (snapshot index strictly increases on every successful write), so when both are usable and
# disagree the HIGHER snapshot index wins. The mirror is a performance hint only -- direction is always
# re-derived live -- so a stale pick can only cost a fast-path skip. mirror_state_file_to_hdfs's "-put -f" is
# non-atomic, so a read can race a write and see an empty file; _hdfs_mirror_content_is_complete filters that.
_pick_state_mirror() {
    local name="$1" require_prefix="${2:-}"
    PICKED_MIRROR_CONTENT=""
    local source_candidate="" dest_candidate="" source_usable=false dest_usable=false
    if source_candidate="$(_read_hdfs_state_mirror "$name" "$SRC_URI_NS")" && _hdfs_mirror_content_is_complete "$source_candidate"; then
        if [[ "$require_prefix" != "match-prefix" ]] || _state_content_matches_prefix "$source_candidate"; then
            source_usable=true
        fi
    elif [[ -n "${source_candidate:-}" ]]; then
        log "[WARN] [HDFS-STATE-MIRROR] HDFS mirror $name on SOURCE_CLUSTER ($SOURCE_CLUSTER) looks empty/unreadable (likely raced an in-flight write). Ignoring it."
    fi
    if dest_candidate="$(_read_hdfs_state_mirror "$name" "$DST_URI_NS")" && _hdfs_mirror_content_is_complete "$dest_candidate"; then
        if [[ "$require_prefix" != "match-prefix" ]] || _state_content_matches_prefix "$dest_candidate"; then
            dest_usable=true
        fi
    elif [[ -n "${dest_candidate:-}" ]]; then
        log "[WARN] [HDFS-STATE-MIRROR] HDFS mirror $name on DEST_CLUSTER ($DEST_CLUSTER) looks empty/unreadable (likely raced an in-flight write). Ignoring it."
    fi

    if [[ "$source_usable" == "true" ]] && [[ "$dest_usable" == "true" ]]; then
        if [[ "$source_candidate" == "$dest_candidate" ]]; then
            PICKED_MIRROR_CONTENT="$source_candidate"
            log "[INFO] [HDFS-STATE-MIRROR] Found matching HDFS mirrors $name on both SOURCE_CLUSTER and DEST_CLUSTER."
        else
            ...
            if [[ "$source_idx" =~ ^[0-9]+$ ]] && [[ "$dest_idx" =~ ^[0-9]+$ ]] && ((dest_idx > source_idx)); then
                PICKED_MIRROR_CONTENT="$dest_candidate"
                log "[WARN] [HDFS-STATE-MIRROR] Divergent HDFS mirrors $name: SOURCE_CLUSTER has '$source_snap', DEST_CLUSTER has '$dest_snap'. Preferring DEST_CLUSTER's (higher snapshot index = more recent state)."
            else
                PICKED_MIRROR_CONTENT="$source_candidate"
                log "[WARN] [HDFS-STATE-MIRROR] Divergent HDFS mirrors $name: SOURCE_CLUSTER has '$source_snap', DEST_CLUSTER has '$dest_snap'. Preferring SOURCE_CLUSTER's (index comparison inconclusive or SOURCE_CLUSTER's index is >= DEST_CLUSTER's)."
            fi
        fi
    elif [[ "$source_usable" == "true" ]]; then
        PICKED_MIRROR_CONTENT="$source_candidate"
        log "[INFO] [HDFS-STATE-MIRROR] Found HDFS mirror $name on SOURCE_CLUSTER ($SOURCE_CLUSTER) only."
    elif [[ "$dest_usable" == "true" ]]; then
        PICKED_MIRROR_CONTENT="$dest_candidate"
        log "[INFO] [HDFS-STATE-MIRROR] Found HDFS mirror $name on DEST_CLUSTER ($DEST_CLUSTER) only."
    fi
    return 0
}

# Remove this job's legacy state copies for a key after they have been migrated to the prefix-keyed name:
# the local legacy files and the directory-only-keyed HDFS mirrors on both clusters, each ONLY if its content
# belongs to this job's SNAP_PREFIX (another job for the same directory may own it). Without this, clearing
# state as the operator guidance instructs would be silently undone by re-adopting the legacy copy. Best-effort.
_retire_legacy_state() {
    local key="$1"
    local legacy content cluster
    for legacy in "/var/tmp/dr-last-snap-${key}.txt" "/var/tmp/dr-last-snap-${key}-${SNAP_PREFIX}.txt"; do
        [[ -f "$legacy" ]] || continue
        content=$(head -n 1 "$legacy" 2>/dev/null || true)
        if _state_content_matches_prefix "$content"; then
            rm -f "$legacy" 2>/dev/null && log "[INFO] [STATE] Removed migrated legacy state file $legacy" || true
        fi
    done
    if _key_has_hdfs_glob "$key"; then
        log "[WARN] [HDFS-STATE-MIRROR] Not removing legacy HDFS mirror ${HDFS_STATE_DIR}/dr-last-snap-${key}.txt: the name contains HDFS glob characters. Remove it by hand if it exists."
        return 0
    fi
    for cluster in "$SRC_URI_NS" "$DST_URI_NS"; do
        if content="$(_read_hdfs_state_mirror "dr-last-snap-${key}.txt" "$cluster")" && _state_content_matches_prefix "$content"; then
            if run_as_hdfs hdfs dfs -fs "hdfs://${cluster}" -rm -f "${HDFS_STATE_DIR}/dr-last-snap-${key}.txt" >/dev/null 2>&1; then
                log "[INFO] [HDFS-STATE-MIRROR] Removed migrated legacy HDFS mirror ${HDFS_STATE_DIR}/dr-last-snap-${key}.txt on $cluster"
            else
                log "[WARN] [HDFS-STATE-MIRROR] Could not remove migrated legacy HDFS mirror ${HDFS_STATE_DIR}/dr-last-snap-${key}.txt on $cluster (harmless; remove it by hand before clearing state for a re-baseline)."
            fi
        fi
    done
    return 0
...
# Resolve the on-disk state file path for a directory key, sets RESOLVED_STATE_PATH.
#
# The state file is /var/tmp/$(state_file_name "$key"), mirrored to ${HDFS_STATE_DIR}/$(state_file_name
# "$key") on both clusters. Resolution order:
#   (a) the local file, if present;
#   (b) the HDFS mirrors of that name (see _pick_state_mirror);
#   (c) legacy names from earlier versions of this script -- the local "/var/tmp/dr-last-snap-${key}.txt" and
#       "/var/tmp/dr-last-snap-${key}-${SNAP_PREFIX}.txt", then the "${HDFS_STATE_DIR}/dr-last-snap-${key}.txt"
#       mirrors -- adopted ONLY if the snapshot recorded in them belongs to this job's SNAP_PREFIX, since the
#       directory-only name is shared by every job replicating the directory;
#   (d) nothing anywhere -> the (nonexistent) local path is returned, so the caller's "-f" test reports the
#       directory as brand new.
# In (b) and (c) the content is written to the local file (and re-mirrored under the new name) before
# returning; in (c) the legacy copies are then removed (_retire_legacy_state). Callers that need to distinguish
# "truly brand new" MUST use resolve_state_file_and_check_new().
#
# CALLING CONVENTION (load-bearing): this function logs to stdout, so it must be called as a plain statement
# and its result read from RESOLVED_STATE_PATH -- NEVER as state="$(resolve_state_file "$key")", which would
# capture the log lines as part of the "path" and misclassify a directory with history as brand new.
# Returns 1 (RESOLVED_STATE_PATH empty) if state was found but could not be written locally -- treating that
# as brand new would re-baseline a directory with real replication history.
resolve_state_file() {
    local key="$1"
    local name new_path
    name="$(state_file_name "$key")"
    new_path="/var/tmp/${name}"
    RESOLVED_STATE_PATH=""
    ...
    local adopted_content="" adopted_from="" from_legacy=false
    _pick_state_mirror "$name"
    if [[ -n "$PICKED_MIRROR_CONTENT" ]]; then
        adopted_content="$PICKED_MIRROR_CONTENT"
        adopted_from="HDFS mirror ${HDFS_STATE_DIR}/${name}"
    else
        ...
        for legacy in "/var/tmp/dr-last-snap-${key}.txt" "/var/tmp/dr-last-snap-${key}-${SNAP_PREFIX}.txt"; do
            ...
            if _state_content_matches_prefix "$content"; then
                ...
                adopted_from="legacy local state file $legacy"
                from_legacy=true
                break
            fi
            log "[INFO] [STATE] Ignoring legacy state file $legacy: its snapshot '$content' does not belong to SNAP_PREFIX '$SNAP_PREFIX' (another replication job for this directory)."
        done
        if [[ -z "$adopted_content" ]] && ! _key_has_hdfs_glob "$key"; then
            _pick_state_mirror "dr-last-snap-${key}.txt" "match-prefix"
            if [[ -n "$PICKED_MIRROR_CONTENT" ]]; then
                adopted_content="$PICKED_MIRROR_CONTENT"
                adopted_from="legacy HDFS mirror ${HDFS_STATE_DIR}/dr-last-snap-${key}.txt"
                from_legacy=true
            fi
        fi
    fi

    if [[ -n "$adopted_content" ]]; then
        if write_state_file "$new_path" "$adopted_content" "$key" >/dev/null 2>&1; then
            log "[INFO] [STATE] No local state file for key=$key; adopted '$(printf '%s' "$adopted_content" | sed -n '1p')' from $adopted_from into $new_path."
            if [[ "$from_legacy" == "true" ]]; then
                _retire_legacy_state "$key"
            fi
        else
            log "[ERROR] [STATE] Found state for key=$key in $adopted_from but failed to write local file $new_path (check disk space/permissions on $(dirname "$new_path")). Treating the directory as brand new would be UNSAFE (it has replication history). Failing loudly instead."
            echo "[ERROR] Could not write local state file $new_path for key=$key (check disk space/permissions on $(dirname "$new_path")). Refusing to continue for this key rather than risk misclassifying it as brand-new." >&2
            return 1
        fi
    fi
...
# Read the state mirror file $1 (a file name under HDFS_STATE_DIR) from a specific cluster's HDFS. Returns the raw content
...
_read_hdfs_state_mirror() {
    local name="$1"
    local cluster="$2"
    local uri_path="${HDFS_STATE_DIR}/${name}"
    local out
...
mirror_state_file_to_hdfs() {
    local key="$1"
    local content="$2"
    # The local temp file is the SOURCE argument of "hdfs dfs -put", which also glob-expands, so its name uses
    # the same glob-safe encoding as the state file name rather than the raw key.
    local mirror_tmp
    mirror_tmp="/tmp/pulse_replication_action_mirror_$$_$(state_file_name "$key")"
...
cleanup_old_snapshots() {
    ...
    local cleanup_ls_names="/tmp/pulse_replication_action_cleanup_ls_names_$$.log"
    TEMP_FILES+=("$cleanup_ls_err" "$cleanup_ls_out" "$cleanup_ls_names")
    local cleanup_ls_rc
    ...
    #
    # Ordered by snapshot NUMBER, oldest first, not by the listing's timestamp: "hdfs dfs -ls" shows time only
    # to the minute, and ties sorted as text put "_10" before "_9". Two snapshots in the same minute (e.g. the
    # rollback re-cutting "_9" and step 4e creating "_10" seconds later) would then have the newer one deleted,
    # and with a low SNAP_RETAIN that can remove the last snapshot the clusters share. The name is taken as the
    # last "/" component of the whole line, so directory paths containing spaces are handled too.
    if run_as_hdfs hdfs dfs -fs "hdfs://$cluster" -ls "$d/.snapshot" 2>"$cleanup_ls_err" |
        awk '$1 ~ /^d/' | sed 's#.*/##' | tee "$cleanup_ls_names" |
        grep -E "^$(snap_re "$snap_prefix")_[0-9]+$" |
        awk -F_ '{print $NF, $0}' | sort -n -k1,1 | cut -d' ' -f2- >"$cleanup_ls_out"; then
        :
    fi
    ...
    if [[ "$cleanup_ls_rc" -ne 0 ]]; then
        ...
        rm -f "$cleanup_ls_err" "$cleanup_ls_out" "$cleanup_ls_names" 2>/dev/null || true
        return 0
    fi
    mapfile -t snaps <"$cleanup_ls_out"
    local -a rollback_snaps=()
    mapfile -t rollback_snaps < <(grep -E "^$(snap_re "$snap_prefix")_rollback_[0-9]+$" "$cleanup_ls_names" || true)
    rm -f "$cleanup_ls_err" "$cleanup_ls_out" "$cleanup_ls_names" 2>/dev/null || true
    ...
    now_epoch=$(date +%s)
    for r in ${rollback_snaps[@]+"${rollback_snaps[@]}"}; do
        r_epoch="${r##*_}"
        r_age=$((now_epoch - r_epoch))
        if ((r_age >= ROLLBACK_MARKER_MAX_AGE_SECS)); then
            if run_as_hdfs hdfs dfs -fs "hdfs://$cluster" -deleteSnapshot "$d" "$r" 2>/dev/null; then
                log "[CLEAN] Removed leftover rollback snapshot $r from $cluster_name for $d (${r_age}s old >= ROLLBACK_MARKER_MAX_AGE_SECS=${ROLLBACK_MARKER_MAX_AGE_SECS}s)"
            else
                log "[WARN] Failed to remove leftover rollback snapshot $r from $cluster_name for $d"
            fi
        else
            log "[DEBUG] Keeping rollback snapshot $r on $cluster_name for $d (${r_age}s old, removed after ${ROLLBACK_MARKER_MAX_AGE_SECS}s)"
        fi
    done
}
...
derive_direction_state() {
    ...
    if run_as_hdfs hdfs dfs -fs "hdfs://$SRC_URI_NS" -ls "$d/.snapshot" 2>"$src_ls_err" |
        ...
        grep -E "^$(snap_re "$SNAP_PREFIX")_[0-9]+$" >"$src_ls_out"; then
        :
    fi
    ...
    if run_as_hdfs hdfs dfs -fs "hdfs://$DST_URI_NS" -ls "$d/.snapshot" 2>"$dst_ls_err" |
        awk '$1 ~ /^d/ {print $6, $7, $8}' | sort | awk -F/ '{print $NF}' |
        grep -E "^$(snap_re "$SNAP_PREFIX")_[0-9]+$" >"$dst_ls_out"; then
        :
    fi
...
# whole-directory "-count" can never match between the two clusters once any excluded file exists.
#
# Lists the snapshot recursively ("hdfs dfs -ls -R") and drops every entry whose path matches an exclude
# pattern, plus everything beneath an excluded directory (DistCp does not descend into a filtered-out
# directory). Like DistCp's regex filter, a pattern must match the WHOLE path; each entry is tested in three
# forms -- "/<dir>/.snapshot/<snap>/<rel>", the same prefixed with the cluster URI, and "/<rel>" -- so a
# pattern written against any of them excludes it. The same filtering is applied to both clusters.
#
# Cost: one recursive listing per cluster per call, proportional to the number of entries in the snapshot
# (the unfiltered path is a single "-count" call). Only used when filtering is enabled.
#
# Returns 0 and prints the signature; 1 if the listing failed; 2 if neither python3 nor perl is available
...
snapshot_filtered_signature() {
    ...
    if ! run_as_hdfs hdfs dfs -fs "$cluster_uri" -ls -R "$snapshot_path" >"$listing" 2>/dev/null; then
...
with open(listing) as fh:
    for line in fh:
        parts = line.rstrip("\n").split(None, 7)
        if len(parts) < 8 or parts[0][:1] not in ("d", "-"):
            continue
        path = parts[7]
        if any(path.startswith(x + "/") for x in excluded_dirs):
            continue
        rel = path[len(prefix):] if path.startswith(prefix) else path
        if any(p.fullmatch(c) for p in pats for c in (path, uri + path, rel)):
            if parts[0][:1] == "d":
        ...
        if parts[0][:1] == "d":
            ...
            files += 1
            size += int(parts[4])
print(dirs, files, size)
PYEOF
        ) || rc=$?
    else
        sig=$(perl -e '
            my ($pf, $ls, $prefix, $uri) = @ARGV;
            open(my $p, "<", $pf) or exit 1;
            my @pats = map { chomp; qr/^(?:$_)$/ } grep { /\S/ && !/^#/ } <$p>;
            open(my $l, "<", $ls) or exit 1;
            my (@exd, $dirs, $files, $size); ($dirs, $files, $size) = (0, 0, 0);
            LINE: while (my $line = <$l>) {
                chomp $line;
                my @f = split(" ", $line, 8);
                next if @f < 8 || $f[0] !~ /^[d-]/;
                my $path = $f[7];
                for my $x (@exd) { next LINE if index($path, "$x/") == 0 }
                my $rel = index($path, $prefix) == 0 ? substr($path, length($prefix)) : $path;
                for my $re (@pats) {
                    for my $c ($path, "$uri$path", $rel) {
                        if ($c =~ $re) { push @exd, $path if $f[0] =~ /^d/; next LINE }
                    }
                }
                if ($f[0] =~ /^d/) { $dirs++ } else { $files++; $size += $f[4] }
            }
            print "$dirs $files $size\n";
        ' "$DISTCP_EXCLUDE_FILE" "$listing" "$snapshot_path" "$cluster_uri") || rc=$?
    fi
    rm -f "$listing" 2>/dev/null || true
    [[ "$rc" -eq 0 && -n "$sig" ]] || return 1
    printf '%s' "$sig"
}
...
verify_snapshot_content_parity() {
    ...
    if [[ "${DISTCP_EXCLUDE_ENABLED,,}" == "yes" ]]; then
        local rc
        if src_sig=$(snapshot_filtered_signature "hdfs://$SRC_URI_NS" "$d/.snapshot/$snap"); then
            :
        else
            rc=$?
            if [[ "$rc" -eq 2 ]]; then
...
# Delete all but the newest $2 files among the arguments that follow (given oldest first -- every pruned file
# name starts with a YYYYmmdd_HHMMSS timestamp, so glob expansion sorts them chronologically). $2 = 0 deletes
# all of them; callers skip the call entirely when LOG_RETAIN=-1 ("never delete").
_prune_oldest() {
    local what="$1" keep="$2"
    shift 2
    (($# <= keep)) && return 0
    local remove=$(($# - keep)) f
    for f in "${@:1:remove}"; do
        rm -f "$f" 2>/dev/null && SETUP_LOG_MESSAGES+=("[INFO] Removed old $what: $f") || true
    done
}

# Host of the first NameNode in a "<nn-id>=<host>:<port>,..." list (SRC_NN_HOSTS / DST_NN_HOSTS), without the
# port. Empty if the list is empty or the host has characters outside [A-Za-z0-9._-].
_first_nn_host() {
    local first="${1%%,*}"
    first="${first#*=}"
    first="${first%%:*}"
    [[ "$first" =~ ^[A-Za-z0-9._-]+$ ]] && printf '%s' "$first"
    return 0
}

# Identity of this replication policy, used to name its log directory and its DistCp filter file. Many
# policies commonly share the same two clusters and the default SNAP_PREFIX and differ only in SOURCE_DIRS, so
# the directories are part of the identity:
#   <source>__<dest>__<prefix>__<first dir>[+<N more>]-<hash>
#   - <source>/<dest>: the cluster names without the port. When both clusters use the same nameservice name,
#     the first NameNode host from SRC_NN_HOSTS / DST_NN_HOSTS is appended ("ODP-Phoenix-nn1.example.com") so
#     the two sides, and the forward and failover directions, stay distinct. Without those variables the
#     suffixes are "-src" / "-dst" (such a run exits at argument validation anyway). Two host:port values that
#     differ only in the port keep the port, since the host alone would not tell them apart.
#   - <first dir>: the alphabetically first directory, with any character outside [A-Za-z0-9._-] replaced by
#     "_". "+<N>" counts the other directories.
#   - <hash>: 8 hex digits of a checksum over the cluster names, prefix and the sorted directory list, so
#     reordering SOURCE_DIRS keeps the same identity and shortened components never cause a collision.
# Each component is capped in length so the name stays well below the 255-byte file-name limit.
# Sets POLICY_KEY (always computed) and JOB_LOG_NAME (LOG_JOB_NAME when set, else POLICY_KEY).
POLICY_KEY=""
JOB_LOG_NAME=""
compute_policy_key() {
    [[ -n "$POLICY_KEY" ]] && return 0
    local src="${SOURCE_CLUSTER%%:*}" dst="${DEST_CLUSTER%%:*}"
    if [[ "$src" == "$dst" ]]; then
        if [[ "$SOURCE_CLUSTER" != "$DEST_CLUSTER" ]]; then
            ...
            local src_nn dst_nn
            src_nn="$(_first_nn_host "$SRC_NN_HOSTS")"
            dst_nn="$(_first_nn_host "$DST_NN_HOSTS")"
            src="${src}-${src_nn:-src}"
            dst="${dst}-${dst_nn:-dst}"
        fi
    fi

    local sorted first extra hash
    sorted="$(printf '%s\n' "${SOURCE_DIRS[@]}" | LC_ALL=C sort)"
    first="$(sanitize "${sorted%%$'\n'*}")"
    first="${first//[^A-Za-z0-9._-]/_}"
    extra=$((${#SOURCE_DIRS[@]} - 1))
    hash="$(printf '%s\n%s\n%s\n%s' "$src" "$dst" "$SNAP_PREFIX" "$sorted" | cksum | awk '{printf "%08x", $1}')"

    POLICY_KEY="${src:0:60}__${dst:0:60}__${SNAP_PREFIX:0:40}__${first:0:60}"
    ((extra > 0)) && POLICY_KEY+="+${extra}"
    POLICY_KEY+="-${hash}"
...
_describe_path_access() {
    local p="$1"
    while [[ ! -e "$p" && "$p" != "/" && "$p" != "." ]]; do
        p="$(dirname "$p")"
    done
    # shellcheck disable=SC2012 # one known path, only owner/mode fields are read
    printf '%s' "$(ls -ld "$p" 2>/dev/null | awk -v p="$p" '{print p ": owner " $3 ":" $4 ", mode " $1}'); running as $(id -un 2>/dev/null || echo "uid $(id -u)")"
}

# Resolve LOG_PATH into this run's log file and apply LOG_RETAIN. Sets LOG (this run's log file) and
# LOG_JOB_DIR (this policy's directory, which also holds the rollback-diff and retry-error files; in single-file
# mode it is created only when one of those is written). See the LOG_PATH block near the top for the layout.
# Runs before output is redirected, so its messages are collected in SETUP_LOG_MESSAGES and printed by main once
# the log file is capturing output. Returns 1 (with the reason on stderr) if the log file cannot be created.
SETUP_LOG_MESSAGES=()
LOG_JOB_DIR=""
LOG_MODE=""
# Timestamp-prefixed names shared with the writers of these files; pruned by LOG_RETAIN like the run logs.
ROLLBACK_DIFF_SUFFIX="rollback_diff"
RETRY_ERR_SUFFIX="distcp_retry_err"
...
setup_log_file() {
    ...
    if [[ ! -d "$LOG_PATH" ]] && { [[ "$LOG_PATH" == *.log ]] || [[ -e "$LOG_PATH" ]]; }; then
        # Single-file mode: one file per policy next to LOG_PATH ("<LOG_PATH minus .log>.<policy>.log"), so
        # policies sharing the same LOG_PATH never rotate, append to, or prune each other's file.
        LOG_MODE="file"
        local log_dir
        log_dir="$(dirname "$LOG_PATH")"
        LOG="${LOG_PATH%.log}.${JOB_LOG_NAME}.log"
        LOG_JOB_DIR="${log_dir}/${JOB_LOG_NAME}"
        if [[ -e "$LOG" && ! -f "$LOG" ]]; then
            echo "[ERROR] Log file path exists but is not a regular file: $LOG" >&2
            return 1
        fi
        if ! mkdir -p "$log_dir" 2>/dev/null; then
            echo "[ERROR] Could not create log directory: $log_dir ($(_describe_path_access "$log_dir"))" >&2
            return 1
        fi
        if [[ -f "$LOG" ]]; then
            # Timestamp + PID keeps two rotations in the same second from overwriting each other.
            local backup_log="${LOG}.${ts}_$$"
            if mv "$LOG" "$backup_log" 2>/dev/null; then
                SETUP_LOG_MESSAGES+=("[INFO] Previous log file rotated to: $backup_log")
            else
                SETUP_LOG_MESSAGES+=("[WARN] Could not rotate existing log file $LOG; appending to it")
            fi
        fi
        local -a backups=()
        local b
        # shellcheck disable=SC2231 # TS_GLOB is an unquoted glob pattern (no whitespace)
        for b in "$LOG".$TS_GLOB*; do
            [[ -f "$b" ]] && backups+=("$b")
        done
        if ((LOG_RETAIN > 0)); then
            _prune_oldest "rotated log" "$LOG_RETAIN" ${backups[@]+"${backups[@]}"}
        fi
    else
        # Directory mode: one subdirectory per policy, one file per run.
        LOG_MODE="dir"
        LOG_JOB_DIR="${LOG_PATH%/}/${JOB_LOG_NAME}"
        if ! mkdir -p "$LOG_JOB_DIR" 2>/dev/null; then
            echo "[ERROR] Could not create log directory: $LOG_JOB_DIR ($(_describe_path_access "$LOG_JOB_DIR"))" >&2
            return 1
        fi
        LOG="${LOG_JOB_DIR}/${ts}_$$.log"
        local -a runs=()
        local r
        # shellcheck disable=SC2231 # TS_GLOB is an unquoted glob pattern (no whitespace)
        for r in "$LOG_JOB_DIR"/${TS_GLOB}_*.log; do
            [[ -f "$r" ]] && runs+=("$r")
        done
        # Keep LOG_RETAIN - 1 previous runs so that, with this run's file, LOG_RETAIN remain.
        if ((LOG_RETAIN > 0)); then
            _prune_oldest "run log" "$((LOG_RETAIN - 1))" ${runs[@]+"${runs[@]}"}
        fi
    fi

    # Rollback-diff and retry-error files: the newest LOG_RETAIN of each kind are kept.
    if ((LOG_RETAIN > 0)); then
        local kind f
        local -a found
        for kind in "$ROLLBACK_DIFF_SUFFIX" "$RETRY_ERR_SUFFIX"; do
            found=()
            # shellcheck disable=SC2231 # TS_GLOB is an unquoted glob pattern (no whitespace)
            for f in "$LOG_JOB_DIR"/${TS_GLOB}_"${kind}"_*.txt; do
                [[ -f "$f" ]] && found+=("$f")
            done
            _prune_oldest "${kind//_/ } file" "$LOG_RETAIN" ${found[@]+"${found[@]}"}
        done
    fi

    if ! touch "$LOG" 2>/dev/null; then
        echo "[ERROR] Could not create log file: $LOG ($(_describe_path_access "$LOG"))" >&2
        return 1
    fi
    if [[ "$LOG_MODE" == "dir" ]]; then
        ln -sfn "$(basename "$LOG")" "$LOG_JOB_DIR/latest.log" 2>/dev/null ||
            SETUP_LOG_MESSAGES+=("[WARN] Could not update $LOG_JOB_DIR/latest.log")
    fi
    return 0
}

# Path for a per-policy artifact file "<LOG_JOB_DIR>/<YYYYmmdd_HHMMSS>_<kind>_<rest>.txt", creating
# LOG_JOB_DIR if needed. Falls back to /tmp if the directory cannot be created, so the caller always gets a
# writable path.
artifact_path() {
    local kind="$1" rest="$2" dir="$LOG_JOB_DIR"
...
#   ${DISTCP_EXCLUDE_DIR}/<POLICY_KEY>.filters
# where POLICY_KEY identifies the policy by clusters, SNAP_PREFIX and SOURCE_DIRS
# (see compute_policy_key).
#
# This guarantees:
#   - Uniqueness across concurrently-running policies (including policies between
#     the same clusters with the same SNAP_PREFIX that differ only in SOURCE_DIRS,
#     each with its own exclude patterns).
...
# per-policy key from the clusters, SNAP_PREFIX and SOURCE_DIRS (see
# compute_policy_key) and (re)writes
# ${DISTCP_EXCLUDE_DIR}/<POLICY_KEY>.filters on every run:
...
rollback_once_for_failure() {
    ...
    if ! run_as_hdfs hdfs dfs -fs "hdfs://$DST_URI_NS" -ls "${d}/.snapshot" 2>/dev/null | grep -q "/$(snap_re "$prev_snap")\$"; then
        log "[WARN] Expected prev_snap '$prev_snap' not found on DR. Choosing latest available snapshot on DR as fallback."
        # Sorted by snapshot number, so the last entry is the highest-numbered one (see cleanup_old_snapshots
        # for why the listing timestamp is not used). Only numbered "${SNAP_PREFIX}_<N>" snapshots are
        # candidates: the listing also contains the
        # "${SNAP_PREFIX}_rollback_<epoch>" snapshot created just above (always the newest), which would
        # otherwise always be selected here and then rejected by the SOURCE_CLUSTER check below.
        mapfile -t snaps_dst < <(
            run_as_hdfs hdfs dfs -fs "hdfs://$DST_URI_NS" -ls "${d}/.snapshot" 2>/dev/null |
                awk '$1 ~ /^d/' | sed 's#.*/##' |
                grep -E "^$(snap_re "$SNAP_PREFIX")_[0-9]+$" |
                awk -F_ '{print $NF, $0}' | sort -n -k1,1 | cut -d' ' -f2- || true
        )
    ...
    if ! run_as_hdfs hdfs dfs -fs "hdfs://$SRC_URI_NS" -ls "${d}/.snapshot" 2>/dev/null | grep -q "/$(snap_re "$prev_snap")\$"; then
    ...
    local diff_out
    # Kept with this policy's run logs (see artifact_path) and pruned by LOG_RETAIN.
    diff_out="$(artifact_path "$ROLLBACK_DIFF_SUFFIX" "${key}_from_${rollback_snap}_to_${prev_snap}")"
    ...
    # DISTCP_ROLLBACK_OPTS carries the -rdiff snapshot pair, then $COPY_OPTS
    # (arg 8) so operator tuning there -- -bandwidth, -m (mapper count),
    # -skipcrccheck, -strategy, -p<flags>, etc. -- also applies to the single
    # most destructive/highest-stakes DistCp job in the script, then the
    # baseline flags a -rdiff restore needs (dynamic strategy, direct write,
    # update semantics, preserve-status flags). DistCp's option parser takes
    # the FIRST occurrence of a repeated option, so COPY_OPTS must come before
    # the baseline for the operator's values to win; the baseline only fills in
    # what COPY_OPTS leaves unset. Generic "-D" tuning from COPY_OPTS
    # (COPY_GENERIC_OPTS) goes in DISTCP_ROLLBACK_FULL_OPTS below, ahead of all
    # DistCp flags, where Hadoop's GenericOptionsParser can see it.
    local DISTCP_ROLLBACK_OPTS="-rdiff $rollback_snap $prev_snap $COPY_OPTS -strategy dynamic -direct -update -pugptx"
    ...
    # this entirely.
    #
    # PUSH mode is the exception: YARN runs on SOURCE_CLUSTER, whose
    # ResourceManager cannot renew DEST_CLUSTER's delegation token -- the same
    # reason the forward push job excludes DEST_CLUSTER. The rollback job only
    # touches DEST_CLUSTER, so it needs that same exclusion or token renewal
    # fails at submission.
    local rollback_token_opts=""
    ...
    if [[ "$rollback_distcp_success" == "true" ]]; then
        ...
        # (only reverting pure file creations leaves no trace). The caller's retry, "distcp -diff $from_snap
        # $to_snap", requires that diff to be empty and would fail again with "target has been modified since
        # snapshot". Re-cutting $from_snap on DEST from the just-restored state satisfies that precondition;
        # the content-parity check after the retry still verifies the result. Only done when the rollback
        # restored to exactly $from_snap -- any other restore point is not the retry's diff base.
        if [[ "$prev_snap" == "$from_snap" ]]; then
            log_substage "Rollback Step 5: Refreshing destination snapshot $from_snap to the restored state"
            if ! run_as_hdfs hdfs dfs -fs "hdfs://$DST_URI_NS" -deleteSnapshot "$d" "$from_snap"; then
                log "[ERROR] [ROLLBACK] Could not delete destination snapshot $from_snap for $d to refresh it; the retry would fail with 'target has been modified since snapshot'. Not retrying."
                return 1
            fi
            if ! run_as_hdfs hdfs dfs -fs "hdfs://$DST_URI_NS" -createSnapshot "$d" "$from_snap"; then
                log "[ERROR] [ROLLBACK] Deleted destination snapshot $from_snap for $d but could not recreate it. Recreate it manually before re-running:"
                log "[ERROR] [ROLLBACK]   hdfs $(render_nameservice_ha_args_for_display)dfs -fs hdfs://$DST_URI_NS -createSnapshot $d $from_snap"
                return 1
            fi
            log "[ROLLBACK] Destination snapshot $from_snap refreshed to the restored state for $d"
        fi
        echo ""
        return 0
    else
...
#   1) One-time full `distcp -update -delete` from the SOURCE ${SNAP_PREFIX}_0 snapshot into the
#      destination. Near-noop if already fully copied; backfills any gaps and deletes destination
#      files that are not in the source snapshot, so the destination mirrors it exactly (extra
#      destination-only files would otherwise be captured in the destination baseline and fail
#      content-parity on every later run). If this fails we do NOT re-baseline (re-baselining a
#      bad state would freeze divergence).
...
reconcile_reverse_diff_bootstrap() {
    ...
    if ! run_as_hdfs hdfs dfs -fs "hdfs://$SRC_URI_NS" -ls "$d/.snapshot" 2>/dev/null | grep -q "/$(snap_re "$last_snap")\$"; then
    ...
    if ! run_as_hdfs hdfs dfs -fs "hdfs://$DST_URI_NS" -ls "$d/.snapshot" 2>/dev/null | grep -q "/$(snap_re "$last_snap")\$"; then
    ...
    if [[ "$bootstrap_distcp_success" != "true" ]]; then
        ...
        if grep -q "The target has been modified since snapshot" "$bootstrap_err" 2>/dev/null ||
           grep -q "target has changed since snapshot" "$bootstrap_err" 2>/dev/null; then
            ...
            echo ""
            echo "      rm -f $state"
            echo "      hdfs ${nameservice_ha_display_args}dfs -fs hdfs://$SRC_URI_NS -rm -f ${HDFS_STATE_DIR}/$(state_file_name "$key")"
            echo "      hdfs ${nameservice_ha_display_args}dfs -fs hdfs://$DST_URI_NS   -rm -f ${HDFS_STATE_DIR}/$(state_file_name "$key")"
            echo ""
...
main() {
    ...
    if ! setup_log_file; then
        echo "[ERROR] Log setup failed for LOG_PATH='$LOG_PATH' (arg 15). Fix the path/permissions and re-run." >&2
        exit 1
    fi
    exec > >(tee -a "$LOG") 2>&1
    local setup_msg
    for setup_msg in ${SETUP_LOG_MESSAGES[@]+"${SETUP_LOG_MESSAGES[@]}"}; do
        log "$setup_msg"
    done
    ...
    if [[ "${DISTCP_EXCLUDE_ENABLED,,}" == "yes" ]]; then
        # DistCp keeps the first -filters it sees, and DISTCP_EXCLUDE_OPTS precedes COPY_OPTS at every call
        # site, so an operator-supplied -filters would be silently ignored.
        if [[ " $COPY_OPTS " == *" -filters "* ]]; then
            echo "[ERROR] COPY_OPTS (arg 8) contains -filters while DistCp exclusion is enabled (DISTCP_EXCLUDE_PATTERNS)." >&2
            echo "[ERROR] Add the patterns to DISTCP_EXCLUDE_PATTERNS instead, or remove -filters from COPY_OPTS." >&2
            exit 23
        fi
        resolve_distcp_exclude_file
    ...
    # Validate ROLLBACK_ON_FAILURE and AUTO_FULL_DISTCP. Both are consumed only as `== "yes"`, so any other
    # value (e.g. "true", "Y") would otherwise silently behave as "no".
    case "${ROLLBACK_ON_FAILURE,,}" in
        ""|no|yes)
            : # valid
            ;;
        *)
            echo "[ERROR] Invalid value for ROLLBACK_ON_FAILURE (arg 11 / env var): '${ROLLBACK_ON_FAILURE}'" >&2
            echo "[ERROR] Accepted values: 'yes' or 'no'" >&2
            exit 24
            ;;
    esac
    case "${AUTO_FULL_DISTCP,,}" in
        ""|no|yes)
            : # valid
            ;;
        *)
            echo "[ERROR] Invalid value for AUTO_FULL_DISTCP (arg 10 / env var): '${AUTO_FULL_DISTCP}'" >&2
            echo "[ERROR] Accepted values: 'yes' or 'no'" >&2
            exit 25
            ;;
    esac
    ...
    echo "  Arg 15 (LOG_PATH)            : $LOG_PATH (this run: $LOG, LOG_RETAIN=$LOG_RETAIN)"
    ...
    # Directories Stage 4 must not process this run, keyed by directory: the bootstrap below baselined them
    # (full copy done, or waiting on a manual full copy), so their first incremental sync belongs to the next
    # run. STAGE4_SKIP_FAILED["$d"]="true" counts the skip as a failure (copy or post-copy snapshot failed).
    # Every other directory still gets its incremental sync this run.
    ...
    for d in "${SOURCE_DIRS[@]}"; do
        ...
        # directory has no replication history left (e.g. its snapshots were deleted for a re-baseline but the
        # state copies were not, or a state file was placed in the shared HDFS_STATE_DIR by something else).
        # Trusting the state would skip the baseline here and fail Stage 4 with "no common snapshot" on every
        # run; instead the directory goes through the brand-new gate below, which confirms against the same
        # live listings before creating anything (and overwrites the stale state on success). Costs two
        # existence checks per directory normally; the listing only runs when both checks fail.
        if [[ "$IS_BRAND_NEW_DIR" != "true" ]]; then
            stale_cached_snap=$(read_state_snapshot "$state" 2>/dev/null || true)
            if [[ -n "$stale_cached_snap" ]] &&
                ! run_as_hdfs hdfs dfs -fs "hdfs://$SRC_URI_NS" -test -e "$d/.snapshot/$stale_cached_snap" 2>/dev/null &&
                ! run_as_hdfs hdfs dfs -fs "hdfs://$DST_URI_NS" -test -e "$d/.snapshot/$stale_cached_snap" 2>/dev/null; then
                derive_direction_state "$d"
                if [[ "$DIRECTION_STATE_OK" == "true" ]] && ((DERIVED_SRC_SNAP_COUNT == 0 && DERIVED_DST_SNAP_COUNT == 0)); then
                    log "[WARN] [STALE-STATE] $d: state records '$stale_cached_snap', but neither cluster has any ${SNAP_PREFIX}_<N> snapshot for this directory. Treating the state as stale and running the brand-new check."
                    IS_BRAND_NEW_DIR="true"
                fi
            fi
        fi
    ...
    if $need_init; then
        ...
        if [[ "${AUTO_FULL_DISTCP,,}" == "yes" ]]; then
            ...
            echo ""
            echo "   The full copy uses -delete: files on the destination that are not in the source"
            echo "   ${SNAP_PREFIX}_0 snapshot are DELETED, so the destination mirrors the source."
            echo ""
            echo "   After successful DistCp, the script will:"
            echo "   - Create a new baseline snapshot on destination (post-DistCp state)"
            echo "   - Start incremental sync for these directories on the NEXT run"
            echo "   - Continue incremental sync for all other configured directories in THIS run"
            echo "   - If DistCp fails for any directory, that directory is reported as failed (non-zero exit)."
            echo ""
            echo "=========================================================================================================================================="
            echo ""
            ...
            DISTCP_ALL_OK=true
            # Directories whose full copy succeeded -- the only ones that get the post-copy destination
            # re-baseline below (a partial copy must never be captured as the destination ${SNAP_PREFIX}_0).
            COPIED_DIRS=()
            ...
            for d in ${BASELINED_DIRS[@]+"${BASELINED_DIRS[@]}"}; do
                # Copy from the source ${SNAP_PREFIX}_0 snapshot, not the live directory: the copy can run for
                # hours, and reading live data would leave the destination holding a mix of _0 content and
                # later writes (and can fail on files that change mid-copy). Same source as
                # reconcile_and_rebaseline_dest; with -update, the snapshot's contents land directly in $d.
                # -delete removes destination files that are not in the source snapshot, so the destination
                # mirrors it exactly: files present only on the destination would otherwise be captured in
                # its ${SNAP_PREFIX}_0 and fail the content-parity check on every incremental run.
                src_uri="hdfs://$SRC_URI_NS${d}/.snapshot/${SNAP_PREFIX}_0"
                dst_uri="hdfs://$DST_URI_NS${d}"
                ...
                distcp_cmd="hadoop distcp $(render_nameservice_ha_args_for_display)$DISTCP_FULL_OPTS $DISTCP_EXCLUDE_OPTS $COPY_OPTS_NO_UPDATE -update -delete $src_uri $dst_uri"
                ...
                if run_as_distcp hadoop distcp $DISTCP_FULL_OPTS $DISTCP_EXCLUDE_OPTS $COPY_OPTS_NO_UPDATE -update -delete "$src_uri" "$dst_uri" 2> >(tee "$DISTCP_STDERR_FILE" >&2); then
                    ...
                    echo ""
                    DISTCP_ALL_OK=false
                    STAGE4_SKIP_REASON["$d"]="full DistCp failed this run"
                    STAGE4_SKIP_FAILED["$d"]="true"
                fi
            done

            # Check if all DistCp operations succeeded. A failed directory is skipped by Stage 4 and counted as
            # failed there; the remaining directories still continue.
            if [[ "$DISTCP_ALL_OK" != "true" ]]; then
                echo ""
                echo "=========================================================================================================================================="
                echo ">>> [ERROR] BASELINE DISTCP FAILED FOR ONE OR MORE DIRECTORIES <<<"
                echo "=========================================================================================================================================="
                echo ""
                ...
                echo "   directory whose copy failed (a partial copy must not be captured as the baseline)."
                ...
                echo ""
                echo "   Directories whose copy succeeded, and all other configured directories, continue"
                echo "   normally in this run. This run exits non-zero."
                echo ""
                log "[ERROR] Baseline full DistCp failed for one or more directories. Those directories are skipped in Stage 4 and counted as failed."
            ...
            for d in ${COPIED_DIRS[@]+"${COPIED_DIRS[@]}"}; do
            ...
            # Scoped to COPIED_DIRS: this block DELETES and recreates the destination's ${SNAP_PREFIX}_0.
            # Run against an already-replicating directory it would destroy that directory's real baseline
            # snapshot and replace it with one capturing unrelated current content; run against a directory
            # whose copy failed it would capture a partial copy as the baseline.
            ...
            for d in ${COPIED_DIRS[@]+"${COPIED_DIRS[@]}"}; do
                ...
                if run_as_hdfs hdfs dfs -fs "hdfs://$DST_URI_NS" -createSnapshot "$d" "$base"; then
                    ...
                    STAGE4_SKIP_REASON["$d"]="full DistCp bootstrap completed this run; incremental sync starts on the next run"
                    STAGE4_SKIP_FAILED["$d"]="false"
                else
                    ...
                    STAGE4_SKIP_REASON["$d"]="post-DistCp baseline snapshot could not be created this run"
                    STAGE4_SKIP_FAILED["$d"]="true"
                fi
            done
            ...
            # diff -- but the run must still surface as failed (STAGE4_SKIP_FAILED above) so it gets noticed.
            if [[ ${#POST_DISTCP_BASELINE_FAILED_DIRS[@]} -gt 0 ]]; then
                ...
                echo "=========================================================================================================================================="
                echo ""
                echo "Full DistCp completed, but the post-DistCp baseline snapshot could not be"
                echo "(re)created on the destination for:"
                ...
                echo "diff. Re-run this script to retry. This run exits non-zero."
                echo ""
                echo "=========================================================================================================================================="
                echo ""
            ...
            if [[ "$DISTCP_ALL_OK" == "true" ]] && [[ ${#POST_DISTCP_BASELINE_FAILED_DIRS[@]} -eq 0 ]]; then
                echo ""
                echo "=========================================================================================================================================="
                ...
                echo "   - Proceed directly to Stage 4 (incremental synchronization) for these directories"
                echo ""
                echo "   Incremental sync for all other configured directories continues below in this run."
                echo ""
                echo "=========================================================================================================================================="
                echo ""
                log "[INFO] All baseline full DistCp operations completed successfully. Post-DistCp snapshots created. These directories start incremental sync on the next run; continuing with the remaining directories."
            fi
            log_stage_complete "3" "Baseline Snapshot Creation"
        else
            # Manual mode: show commands, then continue Stage 4 for the directories that are not waiting on a
            # manual full copy
            ...
            for d in ${BASELINED_DIRS[@]+"${BASELINED_DIRS[@]}"}; do
                has_valid_dirs=true
                STAGE4_SKIP_REASON["$d"]="waiting on the manual full DistCp printed in Stage 3; incremental sync starts on the next run"
                STAGE4_SKIP_FAILED["$d"]="false"
                # From the source ${SNAP_PREFIX}_0 snapshot, not the live directory (see the automatic
                # full-copy loop above for why).
                src_uri="hdfs://$SRC_URI_NS${d}/.snapshot/${SNAP_PREFIX}_0"
                dst_uri="hdfs://$DST_URI_NS${d}"
                ...
                # comment.
                nameservice_ha_display_args="$(render_nameservice_ha_args_for_display)"
                distcp_cmd="hadoop distcp ${nameservice_ha_display_args}$DISTCP_FULL_OPTS $DISTCP_EXCLUDE_OPTS $COPY_OPTS_NO_UPDATE -update -delete $src_uri $dst_uri"
                echo "📁 Directory: $d"
                echo ""
                echo "   Run this command to sync baseline snapshots:"
                echo "   (-delete removes destination files that are not in the source ${SNAP_PREFIX}_0 snapshot)"
                echo ""
                echo "   ==========================================================================================================================================="
                echo "   $distcp_cmd"
                echo "   ==========================================================================================================================================="
    ...
    for d in "${SOURCE_DIRS[@]}"; do
        ...
        if [[ -n "${STAGE4_SKIP_REASON["$d"]:-}" ]]; then
            if [[ "${STAGE4_SKIP_FAILED["$d"]:-false}" == "true" ]]; then
                log "[ERROR] [Stage 4] Skipping $d this run: ${STAGE4_SKIP_REASON["$d"]}. Counted as failed; see the Stage 3 output above."
                METRICS_FAILED_DIRECTORIES=$((METRICS_FAILED_DIRECTORIES + 1))
                ALL_OK=false
            else
                log "[INFO] [Stage 4] Skipping $d this run: ${STAGE4_SKIP_REASON["$d"]}."
            fi
            continue
        fi
        ...
        #      a one-time full `distcp -update -delete` (backfills gaps, removes destination-only
        #      files), then refresh the destination
            ...
            if grep -q "The target has been modified since snapshot" "$DISTCP_STDERR_FILE" 2>/dev/null ||
                ...
                if [[ "${ROLLBACK_ON_FAILURE,,}" == "yes" ]]; then
                    ...
                    if rollback_once_for_failure "$d" "$last_snap" "$next_snap"; then
                        ...
                        # Kept with this policy's run logs for inspection (NOT in TEMP_FILES) and pruned by
                        # LOG_RETAIN. tee writes the retry's stderr to this file AND to stderr.
                        DISTCP_RETRY_STDERR="$(artifact_path "$RETRY_ERR_SUFFIX" "${key}_$$")"
                        # COPY_OPTS_NO_UPDATE is computed once by parse_copy_opts
                        ...
                        if [[ "$DISTCP_RETRY_SUCCESS" == "true" ]]; then
                            ...
                            echo "=========================================================================================================================================="
                            echo ""
                            echo "  Incremental DistCp failed after rollback. State was NOT advanced: it is still '$last_snap',"
                            echo "  and '$next_snap' exists on SOURCE only. Check the retry error above, then choose one option:"
                            echo ""
                            echo "  --- Option 1: Transient failure (network, permissions, NameNode/YARN issue) ---"
                            echo ""
                            echo "    Fix the cause shown in the retry error and re-run this script. It retries the"
                            echo "    incremental sync $last_snap -> $next_snap; nothing needs to be deleted or copied by hand."
                            echo ""
                            echo "  --- Option 2: Full re-sync to '$next_snap' (retry failed again with 'target has been modified since snapshot') ---"
                            echo ""
                            echo "    Makes the destination an exact copy of the source '$next_snap' snapshot, then records that"
                            echo "    point on the destination so the script continues incrementally from '$next_snap'."
                            echo "    Pause anything writing to $d on $DEST_CLUSTER first."
                            echo ""
                            echo "    1. Mirror the source snapshot onto the destination (-delete removes destination files"
                            echo "       that are not in the source snapshot):"
                            echo "         hadoop distcp $(render_nameservice_ha_args_for_display)$DISTCP_FULL_OPTS $DISTCP_EXCLUDE_OPTS $COPY_OPTS_NO_UPDATE -update -delete hdfs://$SRC_URI_NS${d}/.snapshot/$next_snap $dst_uri"
                            echo "    2. Only after step 1 succeeds, create the matching snapshot on the destination:"
                            echo "         hdfs ${nameservice_ha_display_args}dfs -fs hdfs://$DST_URI_NS -createSnapshot $d $next_snap"
                            echo "    3. Re-run this script. It finds '$next_snap' on both clusters and continues from there."
                            echo ""
                            echo "    Do NOT delete '$next_snap' from the source, and do NOT copy from the live source"
                            echo "    directory: either leaves the destination ahead of its last snapshot, so the next run"
                            echo "    fails the same way (and, with ROLLBACK_ON_FAILURE=yes, rolls the copy back)."
                            echo ""
                            echo "  --- Option 3: Inspect .snapshot dirs on both clusters ---"
                            echo ""
                            ...
                            echo "      hdfs ${nameservice_ha_display_args}dfs -fs hdfs://$DST_URI_NS -ls $d/.snapshot"
                            echo ""
                            echo "    What changed on the destination since '$last_snap':"
                            echo "      hdfs ${nameservice_ha_display_args}snapshotDiff -fs hdfs://$DST_URI_NS $d $last_snap ."
                            echo ""
                            echo "=========================================================================================================================================="
                            dir_end_ts=$(date +%s)
        ...
        if ! verify_snapshot_content_parity "$d" "$next_snap"; then
            ...
            ALL_OK=false
            # Retention still applies. '$next_snap' now exists on both clusters, so the next run's live
            # direction check continues from it and creates another pair; if the mismatch persists, skipping
            # cleanup here would add one snapshot pair per run indefinitely (each pinning deleted data).
            # Retention keeps the highest-numbered snapshots, which include '$next_snap' on both clusters.
            cleanup_old_snapshots "$SRC_URI_NS" "$d" "source" "$SNAP_RETAIN" "$SNAP_PREFIX"
            cleanup_old_snapshots "$DST_URI_NS" "$d" "destination" "$SNAP_RETAIN" "$SNAP_PREFIX"
            continue
        fi
...
#
#   [1] <LOG_PATH>/<policy>/<YYYYmmdd_HHMMSS>_<pid>.log
#         (default LOG_PATH: /var/log/hadoop-replication; <policy> from
#         compute_policy_key, or LOG_JOB_NAME)
#         Full run log, one file per run; latest.log in the same directory
#         points at the current/most recent run. Kept forever by default;
#         with LOG_RETAIN=N only the newest N are kept, older ones deleted at
#         startup. In single-file mode (LOG_PATH ending in ".log"), each policy
#         writes <LOG_PATH without .log>.<policy>.log, rotated to
#         <that file>.<YYYYmmdd_HHMMSS>_<pid> (LOG_RETAIN applies to those
#         backups the same way).
#
...
#
#   [3] /var/tmp/dr-last-snap-<SNAP_PREFIX>@<sanitized_dir>.txt   *** STATE FILE ***
...
#
#   [5] <policy log directory>/<YYYYmmdd_HHMMSS>_rollback_diff_<dir>_from_<snap>_to_<snap>.txt
#         snapshotDiff output captured for audit, written every rollback.
#         With LOG_RETAIN=N the newest N are kept.
#
#   [6] <policy log directory>/<YYYYmmdd_HHMMSS>_distcp_retry_err_<dir>_<pid>.txt
#         Retry-attempt DistCp stderr after a rollback, kept for inspection.
#         With LOG_RETAIN=N the newest N are kept.
#
...
#       hadoop distcp -update -delete (full)  last_snap == ${SNAP_PREFIX}_0.
...
#
#   [4] Stage 3 full baseline DistCp          AUTO_FULL_DISTCP="yes" (arg 10),
#       hadoop distcp -update -delete (full)  or the equivalent commands
#                                              printed for manual execution.
#         Copies source into the destination's LIVE path — overwrites
#         destination files that differ from the source AND DELETES
#         destination files not present in the source.
#
