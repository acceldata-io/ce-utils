...
class ODP33RANGERServiceAdvisor(service_advisor.RangerServiceAdvisor):

  def __init__(self, *args, **kwargs):
    self.as_super = super(ODP33RANGERServiceAdvisor, self)
    self.as_super.__init__(*args, **kwargs)

  def getServiceConfigurationRecommendations(self, configurations, clusterData, services, hosts):
    super(ODP33RANGERServiceAdvisor, self).getServiceConfigurationRecommendations(configurations, clusterData, services, hosts)

    recommender = ODP33RangerRecommender()
    recommender.recommendRangerConfigurationsFromODP33(configurations, clusterData, services, hosts)

class ODP33RangerRecommender(service_advisor.ServiceAdvisor):
  """
  Ranger Recommender suggests properties when adding the service for the first time or modifying configs via the UI.
  """

  def __init__(self, *args, **kwargs):
    self.as_super = super(ODP33RangerRecommender, self)
    self.as_super.__init__(*args, **kwargs)
    self.initialize_logger("ODP33RangerRecommender")

  def recommendRangerConfigurationsFromODP33(self, configurations, clusterData, services, hosts):
