...
def run():
    ...
    with SSHClient(host=knox_host) as ssh:
        ...
        if exit_code != 0:
            ...
            print("     - Knox keystore: ls -la /usr/odp/current/knox-server/data/security/keystores/")
            print("  2. Try exporting manually:")
        ...
        if "-----BEGIN CERTIFICATE-----" in pem_content and "-----END CERTIFICATE-----" in pem_content:
            print("  Format: ✓ Valid PEM certificate")
        else:
            print("  Format: ⚠ WARNING - Does not look like a valid PEM certificate!")
