...
from knox_setup.modules import get_ambari_state, SSHClient
from knox_setup.modules.service_utils import ServiceUtils
from knox_setup.modules.knox_state import get_knox_state
from knox_setup.steps.base import get_ambari_configs
from knox_setup.steps import export_knox_cert
from knox_setup.config.settings import AmbariConfig


def run(protocol: str = "https"):
    """
    ...
    sso_provider_url = f"{protocol}://{knox_host}:8443/gateway/knoxsso/api/v1/websso"
    ...
    with SSHClient(host=ambari_host) as ssh:
        ...
        if need_to_write_cert:
            ...
            if exit_code != 0:
                print("  ✗ ERROR: Failed to write certificate file")
            ...
            if exit_code == 0 and "BEGIN CERTIFICATE" in stdout:
                ...
                print("  ✗ ERROR: Certificate file verification failed")
        ...
        print("  ✓ SSO enabled: true")
        print("  ✓ SSO enabled for Ambari: true")
        print("  ✓ SSO manage services: true")
        print("  ✓ SSO enabled services: * (all)")
        print(f"  ✓ Provider URL: {sso_provider_url}")
        print(f"  ✓ Certificate: {cert_file_path}")
        print("  ✓ Cookie name: hadoop-jwt")
        print("")
