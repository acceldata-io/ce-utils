...
def import_cert_on_host(host: str, knox_cert_pem: str, alias: str = "knox-gateway") -> bool:
    ...
    try:
        with SSHClient(host=host) as ssh:
            ...
            if exit_code != 0:
                print("    ✗ Failed to write certificate")
                return False
...
def get_ranger_hosts(service_utils: ServiceUtils) -> list:
    ...
    if ranger_kms_host:
        ...
        print("    - RANGER_KMS_SERVER: not found")
...
def run(protocol: str = "http"):
    ...
    if results['success']:
        ...
        try:
            with SSHClient(host=first_host) as ssh:
                ...
                if stderr.strip():
                    print(f"STDERR output: {stderr.strip()}", file=sys.stderr)
        except Exception as e:
    ...
    if cacerts_path_for_config:
        ...
        print("  Setting ranger-admin-site properties:")
