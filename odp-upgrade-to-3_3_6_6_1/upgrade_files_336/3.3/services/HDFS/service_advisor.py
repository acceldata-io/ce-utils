...
class ODP33HDFSServiceAdvisor(service_advisor.HDFSServiceAdvisor):

  def __init__(self, *args, **kwargs):
    self.as_super = super(ODP33HDFSServiceAdvisor, self)
    self.as_super.__init__(*args, **kwargs)

    self.initialize_logger("ODP33HDFSServiceAdvisor")
  ...
  def getServiceConfigurationRecommendations(self, configurations, clusterData, services, hosts):
    super(ODP33HDFSServiceAdvisor, self).getServiceConfigurationRecommendations(configurations, clusterData, services, hosts)

    recommender = ODP33HDFSRecommender()
    recommender.recommendHDFSConfigurationsFromODP33(configurations, clusterData, services, hosts)

class ODP33HDFSRecommender(service_advisor.ServiceAdvisor):
  """
  HDFS Recommender suggests properties when adding the service for the first time or modifying configs via the UI.
  """

  def __init__(self, *args, **kwargs):
    self.as_super = super(ODP33HDFSRecommender, self)
    self.as_super.__init__(*args, **kwargs)

  def recommendHDFSConfigurationsFromODP33(self, configurations, clusterData, services, hosts):
